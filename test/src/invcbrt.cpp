...
std::complex<T> invcbrt(std::complex<T> z){
    ...
    return std::complex<T>{1.0, 0.0} / std::complex<T>{new_abs * std::cos(new_arg), new_abs * std::sin(new_arg)};
}

TEST_CASE("invcbrt/s/0", "[float][invcbrt]") {
    const size_t N = 137;
    ...
    egblas_sinvcbrt(N, 1.0, x_gpu, 1, y_gpu, 1);
    ...
    for (size_t i = 1; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0f * (1.0f / std::cbrt(1.0f * i))));
    }
...
TEST_CASE("invcbrt/s/1", "[float][invcbrt]") {
    const size_t N = 333;
    ...
    egblas_sinvcbrt(N, 0.2f, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2f * (1.0f / std::cbrt(1.1f * (i + 1)))));
    }
...
TEST_CASE("invcbrt/d/0", "[double][invcbrt]") {
    const size_t N = 137;
    ...
    egblas_dinvcbrt(N, 1.0, x_gpu, 1, y_gpu, 1);
    ...
    for (size_t i = 1; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0 * (1.0 / std::cbrt(1.0 * i))));
    }
...
TEST_CASE("invcbrt/d/1", "[double][invcbrt]") {
    const size_t N = 333;
    ...
    egblas_dinvcbrt(N, 0.2, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2 * (1.0 / std::cbrt(1.1 * (i + 1)))));
    }
...
TEST_CASE("invcbrt/c/0", "[float][invcbrt]") {
    const size_t N = 137;
    ...
    egblas_cinvcbrt(N, make_cuComplex(1.0f, 0.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, reinterpret_cast<cuComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(invcbrt(x_cpu[i])));
    }
...
TEST_CASE("invcbrt/c/1", "[float][invcbrt]") {
    const size_t N = 338;
    ...
    egblas_cinvcbrt(N, make_cuComplex(1.0f, 1.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, reinterpret_cast<cuComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(std::complex<float>(1.0f, 1.0f) * invcbrt(x_cpu[i])));
    }
...
TEST_CASE("invcbrt/z/0", "[double][invcbrt]") {
    const size_t N = 137;
    ...
    egblas_zinvcbrt(N, make_cuDoubleComplex(1.0, 0.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(invcbrt(x_cpu[i])));
    }
...
TEST_CASE("invcbrt/z/1", "[double][invcbrt]") {
    const size_t N = 338;
    ...
    egblas_zinvcbrt(N, make_cuDoubleComplex(0.1, 2.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(std::complex<double>(0.1, 2.0) * invcbrt(x_cpu[i])));
    }
