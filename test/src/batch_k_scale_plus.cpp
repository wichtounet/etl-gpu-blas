...
TEST_CASE("batch_k_scale_plus2/s/0", "[float][batch_k_scale_plus2]") {
    const size_t B = 2;
    const size_t K = 3;
...
TEST_CASE("batch_k_scale_plus2/s/1", "[float][batch_k_scale_plus2]") {
    const size_t B = 3;
    const size_t K = 2;
...
TEST_CASE("batch_k_scale_plus2/d/0", "[double][batch_k_scale_plus2]") {
    const size_t B = 2;
    const size_t K = 3;
    ...
    egblas_dbatch_k_scale_plus2(B, K, x_gpu, gamma_gpu, beta_gpu, y_gpu);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, B * K * sizeof(double), cudaMemcpyDeviceToHost));

    REQUIRE(y_cpu[0] == 1.0);
    REQUIRE(y_cpu[1] == 5.0);
    REQUIRE(y_cpu[2] == 11.0);
    REQUIRE(y_cpu[3] == 4.0);
    REQUIRE(y_cpu[4] == 11.0);
    REQUIRE(y_cpu[5] == 20.0);
...
TEST_CASE("batch_k_scale_plus2/d/1", "[double][batch_k_scale_plus2]") {
    const size_t B = 2;
    const size_t K = 4;
    ...
    REQUIRE(y_cpu[1] == 5.0);
    REQUIRE(y_cpu[2] == 11.0);
    REQUIRE(y_cpu[3] == 19.0);
    REQUIRE(y_cpu[4] == 5.0);
    REQUIRE(y_cpu[5] == 13.0);
    REQUIRE(y_cpu[6] == 23.0);
    REQUIRE(y_cpu[7] == 35.0);
...
TEST_CASE("batch_k_scale_plus4/s/0", "[float][batch_k_scale_plus2]") {
    const size_t B = 5;
    const size_t K = 3;
    const size_t M = 4;
    const size_t N = 4;
    ...
    egblas_sbatch_k_scale_plus4(B, K, M, N, x_gpu, gamma_gpu, beta_gpu, y_gpu);
...
TEST_CASE("batch_k_scale_plus4/d/0", "[float][batch_k_scale_plus2]") {
    const size_t B = 2;
    const size_t K = 3;
    const size_t M = 4;
    const size_t N = 5;
    ...
    egblas_dbatch_k_scale_plus4(B, K, M, N, x_gpu, gamma_gpu, beta_gpu, y_gpu);
