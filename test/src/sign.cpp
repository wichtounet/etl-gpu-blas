...
template <typename T>
std::complex<T> signc(std::complex<T> x) {
    if(x.real() == 0.0 && x.imag() == 0.0){
        return std::complex<T>(0, 0);
    } else {
        return x / std::complex<T>(std::abs(x), 0);
    }
}

} // anonymous namespace

TEST_CASE("sign/s/0", "[float][sign]") {
    const size_t N = 137;
    ...
    egblas_ssign(N, 1.0, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0f * sign(i - 28.0f)));
    }
...
TEST_CASE("sign/s/1", "[float][sign]") {
    ...
    egblas_ssign(N, 0.2f, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2f * sign(1.1f * (i - 24))));
    }
...
TEST_CASE("sign/d/0", "[double][sign]") {
    const size_t N = 137;
    ...
    egblas_dsign(N, 1.0, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0 * sign(i - 19.5f)));
    }
...
TEST_CASE("sign/d/1", "[double][sign]") {
    const size_t N = 333;
    ...
    egblas_dsign(N, 0.2, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2 * sign(1.1 * (i - 99.9f))));
    }
...
TEST_CASE("sign/c/0", "[float][sign]") {
    ...
    egblas_csign(N, make_cuComplex(1.0f, 0.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, reinterpret_cast<cuComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(signc(std::complex<float>(1.0f * (i / 1000.0f), 1.0f * i))));
    }
...
TEST_CASE("sign/c/1", "[float][sign]") {
    ...
    egblas_csign(N, make_cuComplex(1.0f, 1.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, reinterpret_cast<cuComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(std::complex<float>(1.0f, 1.0f) * signc(std::complex<float>(i / 998.0f, -2.0f * i))));
    }
...
TEST_CASE("sign/z/0", "[double][sign]") {
    const size_t N = 137;
    ...
    egblas_zsign(N, make_cuDoubleComplex(1.0, 0.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(signc(std::complex<double>(i / 999.0, -1.1 * i))));
    }
...
TEST_CASE("sign/z/1", "[double][sign]") {
    const size_t N = 338;
    ...
    egblas_zsign(N, make_cuDoubleComplex(0.1, 2.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(std::complex<double>(0.1, 2.0) * signc(std::complex<double>(i / 996.0, -2.4 * i))));
    }
