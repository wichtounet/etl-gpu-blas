...
TEST_CASE("axpby/s/0", "[float][axpby]") {
    ...
    egblas_saxpby(N, 1.0, x_gpu, 1, 1.0, y_gpu, 1);
...
TEST_CASE("axpby/s/1", "[float][axpby]") {
    ...
    egblas_saxpby(N, 0.2, x_gpu, 1, 0.3, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2f * i + 0.3 * 2.3 * i));
    }
...
TEST_CASE("axpby/s/2", "[float][axpby]") {
    ...
    egblas_saxpby(N, 0.2, x_gpu, 3, 0.5, y_gpu, 3);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        if (i % 3 == 0) {
            REQUIRE(y_cpu[i] == Approx(0.2f * i + 0.5f * 2.3f * i));
        } else {
...
TEST_CASE("axpby/d/0", "[double][axpby]") {
    ...
    egblas_daxpby(N, 1.0, x_gpu, 1, 1.1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0f * i + 1.1 * 2.1f * i));
    }
...
TEST_CASE("axpby/d/1", "[double][axpby]") {
    ...
    egblas_daxpby(N, 0.2, x_gpu, 1, 1.0, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2f * i + 1.0 * 2.3 * i));
    }
...
TEST_CASE("axpby/d/2", "[double][axpby]") {
    ...
    egblas_daxpby(N, 0.2, x_gpu, 3, 3.0, y_gpu, 3);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        if (i % 3 == 0) {
            REQUIRE(y_cpu[i] == Approx(0.2f * i + 3.0 * 2.3f * i));
        } else {
            REQUIRE(y_cpu[i] == Approx(2.3f * i));
        }
    }
...
TEST_CASE("axpby/c/1", "[float][axpby]") {
    ...
    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(std::complex<float>(i, -1.0f * i) + std::complex<float>(1.0, 0.1) * std::complex<float>(-1.0f * i, 2.1f * i)));
    }
...
TEST_CASE("axpby/z/0", "[double][axpby]") {
    ...
    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(std::complex<double>(i, i) + std::complex<double>(-2.0f * i, 0.1f * i)));
    }
...
TEST_CASE("axpby/z/1", "[double][axpby]") {
    ...
    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(std::complex<double>(-1.0, 0.2) * std::complex<double>(i, -1.0 * i) + std::complex<double>(1.1, 0.1) * std::complex<double>(-1.0 * i, 2.1 * i)));
    }
