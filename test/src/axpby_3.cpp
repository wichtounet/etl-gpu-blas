...
TEST_CASE("axpby_3/s/0", "[float][axpby_3]") {
    ...
    egblas_saxpby_3(N, 1.0, x_gpu, 1, 1.0, y_gpu, 1, yy_gpu, 1);
...
TEST_CASE("axpby_3/s/1", "[float][axpby_3]") {
    ...
    egblas_saxpby_3(N, 0.2, x_gpu, 1, 0.3, y_gpu, 1, yy_gpu, 1);

    cuda_check(cudaMemcpy(yy_cpu, yy_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(yy_cpu[i] == Approx(0.2f * i + 0.3 * 2.3 * i));
    }
...
TEST_CASE("axpby_3/s/2", "[float][axpby_3]") {
    ...
    cuda_check(cudaMemcpy(x_gpu, x_cpu, N * sizeof(float), cudaMemcpyHostToDevice));
    cuda_check(cudaMemcpy(y_gpu, y_cpu, N * sizeof(float), cudaMemcpyHostToDevice));
    cuda_check(cudaMemcpy(yy_gpu, yy_cpu, N * sizeof(float), cudaMemcpyHostToDevice));

    egblas_saxpby_3(N, 0.2, x_gpu, 3, 0.5, y_gpu, 3, yy_gpu, 3);

    cuda_check(cudaMemcpy(yy_cpu, yy_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        if (i % 3 == 0) {
            REQUIRE(yy_cpu[i] == Approx(0.2f * i + 0.5f * 2.3f * i));
        } else {
            REQUIRE(yy_cpu[i] == Approx(0.3f * i));
        }
    }
...
TEST_CASE("axpby_3/d/0", "[double][axpby_3]") {
    ...
    egblas_daxpby_3(N, 1.0, x_gpu, 1, 1.1, y_gpu, 1, yy_gpu, 1);

    cuda_check(cudaMemcpy(yy_cpu, yy_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(yy_cpu[i] == Approx(1.0f * i + 1.1 * 2.1f * i));
    }
...
TEST_CASE("axpby_3/d/1", "[double][axpby_3]") {
    ...
    egblas_daxpby_3(N, 0.2, x_gpu, 1, 1.0, y_gpu, 1, yy_gpu, 1);

    cuda_check(cudaMemcpy(yy_cpu, yy_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(yy_cpu[i] == Approx(0.2 * i + 1.0 * 2.3 * i));
    }
...
TEST_CASE("axpby_3/d/2", "[double][axpby_3]") {
    ...
    cuda_check(cudaMemcpy(x_gpu, x_cpu, N * sizeof(double), cudaMemcpyHostToDevice));
    cuda_check(cudaMemcpy(y_gpu, y_cpu, N * sizeof(double), cudaMemcpyHostToDevice));
    cuda_check(cudaMemcpy(yy_gpu, yy_cpu, N * sizeof(double), cudaMemcpyHostToDevice));

    egblas_daxpby_3(N, 0.2, x_gpu, 3, 3.0, y_gpu, 3, yy_gpu, 3);

    cuda_check(cudaMemcpy(yy_cpu, yy_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        if (i % 3 == 0) {
            REQUIRE(yy_cpu[i] == Approx(0.2f * i + 3.0 * 2.3f * i));
        } else {
            REQUIRE(yy_cpu[i] == Approx(i + 1.0f));
        }
    }
...
TEST_CASE("axpby_3/c/0", "[float][axpby_3]") {
    ...
    egblas_caxpby_3(N,
        make_cuComplex(1.0, 0.0), reinterpret_cast<cuComplex*>(x_gpu), 1,
        make_cuComplex(1.0, 0.0), reinterpret_cast<cuComplex*>(y_gpu), 1,
        reinterpret_cast<cuComplex*>(yy_gpu), 1);

    cuda_check(cudaMemcpy(yy_cpu, yy_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(yy_cpu[i] == TestComplex<float>(std::complex<float>(i, i) + std::complex<float>(-2.0f * i, 0.1f * i)));
    }
...
TEST_CASE("axpby_3/c/1", "[float][axpby_3]") {
    ...
    for (size_t i = 0; i < N; ++i) {
        x_cpu[i] = std::complex<float>(i, -1.0f * i);
    ...
    egblas_caxpby_3(N,
        make_cuComplex(1.0, 0.0), reinterpret_cast<cuComplex*>(x_gpu), 1,
        make_cuComplex(1.0, 0.1), reinterpret_cast<cuComplex*>(y_gpu), 1,
        reinterpret_cast<cuComplex*>(yy_gpu), 1);

    cuda_check(cudaMemcpy(yy_cpu, yy_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(yy_cpu[i] == TestComplex<float>(std::complex<float>(i, -1.0f * i) + std::complex<float>(1.0, 0.1) * std::complex<float>(-1.0f * i, 2.1f * i)));
    }
...
TEST_CASE("axpby_3/z/0", "[double][axpby_3]") {
    ...
    egblas_zaxpby_3(N,
        make_cuDoubleComplex(1.0, 0.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1,
        make_cuDoubleComplex(1.0, 0.0), reinterpret_cast<cuDoubleComplex*>(y_gpu), 1,
        reinterpret_cast<cuDoubleComplex*>(yy_gpu), 1);

    cuda_check(cudaMemcpy(yy_cpu, yy_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(yy_cpu[i] == TestComplex<double>(std::complex<double>(i, i) + std::complex<double>(-2.0f * i, 0.1f * i)));
    }
...
TEST_CASE("axpby_3/z/1", "[double][axpby_3]") {
    ...
    for (size_t i = 0; i < N; ++i) {
        ...
        y_cpu[i] = std::complex<double>(-1.0 * i, 2.1 * i);
    }
    ...
    egblas_zaxpby_3(N,
        make_cuDoubleComplex(-1.0, 0.2), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1,
        make_cuDoubleComplex(1.1, 0.1), reinterpret_cast<cuDoubleComplex*>(y_gpu), 1,
        reinterpret_cast<cuDoubleComplex*>(yy_gpu), 1);

    cuda_check(cudaMemcpy(yy_cpu, yy_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(yy_cpu[i] == TestComplex<double>(std::complex<double>(-1.0, 0.2) * std::complex<double>(i, -1.0 * i) + std::complex<double>(1.1, 0.1) * std::complex<double>(-1.0 * i, 2.1 * i)));
    }
