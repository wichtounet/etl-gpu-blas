...
TEST_CASE_TEMPLATE("axpby_3/i/0", T, int32_t, int64_t) {
    const size_t N = 137;
    ...
    if constexpr (std::is_same_v<T, int32_t>) {
        ...
        egblas_laxpby_3(N, 1, x.gpu(), 1, 1, y.gpu(), 1, yy.gpu(), 1);
    }
...
TEST_CASE_TEMPLATE("axpby_3/i/1", T, int32_t, int64_t) {
    const size_t N = 333;
    ...
    if constexpr (std::is_same_v<T, int32_t>) {
        ...
        egblas_laxpby_3(N, 2, x.gpu(), 1, 3, y.gpu(), 1, yy.gpu(), 1);
    }
...
TEST_CASE_TEMPLATE("axpby_3/i/2", T, int32_t, int64_t) {
    const size_t N = 111;
    ...
    if constexpr (std::is_same_v<T, int32_t>) {
        ...
        egblas_laxpby_3(N, 2, x.gpu(), 3, 5, y.gpu(), 3, yy.gpu(), 3);
    }
...
TEST_CASE_HALF("axpby_3/h/0") {
    ...
    if constexpr (std::is_same_v<T, fp16>) {
        egblas_haxpby_3(N, fromFloat<T>(1.0), x.gpu(), 1, fromFloat<T>(1.0f), y.gpu(), 1, yy.gpu(), 1);
    } else if constexpr (std::is_same_v<T, bf16>) {
        egblas_baxpby_3(N, fromFloat<T>(1.0), x.gpu(), 1, fromFloat<T>(1.0f), y.gpu(), 1, yy.gpu(), 1);
    }
...
TEST_CASE_HALF("axpby_3/h/1") {
    ...
    if constexpr (std::is_same_v<T, fp16>) {
        egblas_haxpby_3(N, fromFloat<T>(-0.1f), x.gpu(), 1, fromFloat<T>(0.2f), y.gpu(), 1, yy.gpu(), 1);
    } else if constexpr (std::is_same_v<T, bf16>) {
        egblas_baxpby_3(N, fromFloat<T>(-0.1f), x.gpu(), 1, fromFloat<T>(0.2f), y.gpu(), 1, yy.gpu(), 1);
    }
...
TEST_CASE_HALF("axpby_3/h/2") {
    ...
    if constexpr (std::is_same_v<T, fp16>) {
        egblas_haxpby_3(N, fromFloat<T>(0.1f), x.gpu(), 1, fromFloat<T>(-0.1f), y.gpu(), 1, yy.gpu(), 1);
    } else if constexpr (std::is_same_v<T, bf16>) {
        egblas_baxpby_3(N, fromFloat<T>(0.1f), x.gpu(), 1, fromFloat<T>(-0.1f), y.gpu(), 1, yy.gpu(), 1);
    }

    yy.gpu_to_cpu();

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(__high2float(yy.cpu()[i]) == Approx(0.1f * (i + 1.0f) + (-0.1f * (4.1f * i + 0.1f))).epsilon(half_eps));
        REQUIRE(__low2float(yy.cpu()[i]) == Approx(0.1f * (i + 1.0f) + (-0.1f * (4.1f * i + 0.1f))).epsilon(half_eps));
    }
}
