...
TEST_CASE("apxdbpy/s/0", "[float][apxdbpy]") {
    const size_t N = 137;
    ...
    egblas_sapxdbpy(N, 1.01f, x_gpu, 1, 2.0f, y_gpu, 1);
...
TEST_CASE("apxdbpy/d/0", "[double][apxdbpy]") {
    const size_t N = 137;
    ...
    egblas_dapxdbpy(N, 1.4, x_gpu, 1, 1.5, y_gpu, 1);
...
TEST_CASE("apxdbpy/c/0", "[float][apxdbpy]") {
    const size_t N = 99;
    ...
    egblas_capxdbpy(N, make_cuComplex(1.0f, 2.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, make_cuComplex(1.01f, 2.01f), reinterpret_cast<cuComplex*>(y_gpu), 1);
...
TEST_CASE("apxdbpy/z/0", "[double][apxdbpy]") {
    const size_t N = 99;
    ...
    egblas_zapxdbpy(N, make_cuDoubleComplex(1.0f, 2.0f), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, make_cuDoubleComplex(1.01f, 2.01f), reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);
...
TEST_CASE_HALF("apxdbpy/h/0") {
    ...
    if constexpr (std::is_same_v<T, fp16>) {
        egblas_hapxdbpy(N, fromFloat<T>(1.0), x_gpu, 1, fromFloat<T>(1.0), y_gpu, 1);
    } else if constexpr (std::is_same_v<T, bf16>) {
        egblas_bapxdbpy(N, fromFloat<T>(1.0), x_gpu, 1, fromFloat<T>(1.0), y_gpu, 1);
    }

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(T), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(__high2float(y_cpu[i]) == Approx((1.0f + i + 0.1f) / (1.0f + 2.1f * i + 0.1f)).epsilon(half_eps));
        REQUIRE(__low2float(y_cpu[i]) == Approx((1.0f + i + 0.1f) / (1.0f + 2.1f * i + 0.1f)).epsilon(half_eps));
    }
...
TEST_CASE_HALF("apxdbpy/h/1") {
    ...
    if constexpr (std::is_same_v<T, fp16>) {
        egblas_hapxdbpy(N, fromFloat<T>(0.5), x_gpu, 1, fromFloat<T>(1.0), y_gpu, 1);
    } else if constexpr (std::is_same_v<T, bf16>) {
        egblas_bapxdbpy(N, fromFloat<T>(0.5), x_gpu, 1, fromFloat<T>(1.0), y_gpu, 1);
    }
...
TEST_CASE_HALF("apxdbpy/h/2") {
    ...
    if constexpr (std::is_same_v<T, fp16>) {
        egblas_hapxdbpy(N, fromFloat<T>(0.0), x_gpu, 1, fromFloat<T>(0.1), y_gpu, 1);
    } else if constexpr (std::is_same_v<T, bf16>) {
        egblas_bapxdbpy(N, fromFloat<T>(0.0), x_gpu, 1, fromFloat<T>(0.1), y_gpu, 1);
    }

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(T), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(__high2float(y_cpu[i]) == Approx(i / (0.1 + -2.1f * i)).epsilon(half_eps));
        REQUIRE(__low2float(y_cpu[i]) == Approx(i / (0.1 + -2.1f * i)).epsilon(half_eps));
    }
