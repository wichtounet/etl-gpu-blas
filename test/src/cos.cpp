...
TEST_CASE("cos/s/0", "[float][cos]") {
    ...
    egblas_scos(N, 1.0, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0f * std::cos(x_cpu[i])).epsilon(large_eps));
    }
...
TEST_CASE("cos/s/1", "[float][cos]") {
    ...
    egblas_scos(N, 0.2f, x_gpu, 1, y_gpu, 1);
...
TEST_CASE("cos/d/0", "[double][cos]") {
    ...
    egblas_dcos(N, 1.0, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0 * std::cos(x_cpu[i])));
    }
...
TEST_CASE("cos/d/1", "[double][cos]") {
    ...
    egblas_dcos(N, 0.2, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2 * std::cos(x_cpu[i])));
    }
...
TEST_CASE("cos/c/0", "[float][cos]") {
    ...
    egblas_ccos(N, make_cuComplex(1.0f, 0.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, reinterpret_cast<cuComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(std::cos(x_cpu[i])));
    }
...
TEST_CASE("cos/c/1", "[float][cos]") {
    ...
    for (size_t i = 0; i < N; ++i) {
        x_cpu[i] = std::complex<float>(0.1 + i / 998.0f, 0.1 + -2.0f * i);
    }
    ...
    egblas_ccos(N, make_cuComplex(1.0f, 1.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, reinterpret_cast<cuComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(std::complex<float>(1.0f, 1.0f) * std::cos(x_cpu[i])));
    }
...
TEST_CASE("cos/z/0", "[double][cos]") {
    ...
    egblas_zcos(N, make_cuDoubleComplex(1.0, 0.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(std::cos(x_cpu[i])));
    }
...
TEST_CASE("cos/z/1", "[double][cos]") {
    ...
    egblas_zcos(N, make_cuDoubleComplex(0.1, 2.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(std::complex<double>(0.1, 2.0) * std::cos(x_cpu[i])));
    }
