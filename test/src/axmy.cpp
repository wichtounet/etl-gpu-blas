...
TEST_CASE_TEMPLATE("axmy/i/1", T, int32_t, int64_t) {
    ...
    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y.cpu()[i] == Approx(20 * i * 23 * i));
    }
}
...
TEST_CASE_HALF("axmy/h/0") {
    ...
    if constexpr (std::is_same_v<T, fp16>) {
        egblas_haxmy(N, fromFloat<T>(1.0), x.gpu(), 1, y.gpu(), 1);
    } else if constexpr (std::is_same_v<T, bf16>) {
        egblas_baxmy(N, fromFloat<T>(1.0), x.gpu(), 1, y.gpu(), 1);
    }
...
TEST_CASE_HALF("axmy/h/1") {
    ...
    if constexpr (std::is_same_v<T, fp16>) {
        egblas_haxmy(N, fromFloat<T>(0.2), x.gpu(), 1, y.gpu(), 1);
    } else if constexpr (std::is_same_v<T, bf16>) {
        egblas_baxmy(N, fromFloat<T>(0.2), x.gpu(), 1, y.gpu(), 1);
    }

    y.gpu_to_cpu();

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(__high2float(y.cpu()[i]) == Approx(0.2f * 0.5f * i * 0.1f * i).epsilon(half_eps));
        REQUIRE(__low2float(y.cpu()[i]) == Approx(0.2f * 0.5f * i * 0.1f * i).epsilon(half_eps));
    }
}
...
TEST_CASE_HALF("axmy/h/1") {
    ...
    if constexpr (std::is_same_v<T, fp16>) {
        egblas_haxmy(N, fromFloat<T>(0.1), x.gpu(), 1, y.gpu(), 1);
    } else if constexpr (std::is_same_v<T, bf16>) {
        egblas_baxmy(N, fromFloat<T>(0.1), x.gpu(), 1, y.gpu(), 1);
    }

    y.gpu_to_cpu();

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(__high2float(y.cpu()[i]) == Approx(0.1f * 0.5f * i * 1.1f * i).epsilon(half_eps));
        REQUIRE(__low2float(y.cpu()[i]) == Approx(0.1f * 0.5f * i * 1.1f * i).epsilon(half_eps));
    }
}
