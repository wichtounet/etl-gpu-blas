...
TEST_CASE("apxdby/s/0", "[float][apxdby]") {
    const size_t N = 137;
    ...
    egblas_sapxdby(N, 1.01f, x_gpu, 1, 2.0f, y_gpu, 1);
...
TEST_CASE("apxdby/d/0", "[double][apxdby]") {
    const size_t N = 137;
    ...
    egblas_dapxdby(N, 1.4, x_gpu, 1, 1.5, y_gpu, 1);
...
TEST_CASE("apxdby/c/0", "[float][apxdby]") {
    const size_t N = 99;
    ...
    egblas_capxdby(N, make_cuComplex(1.0f, 2.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, make_cuComplex(1.01f, 2.01f), reinterpret_cast<cuComplex*>(y_gpu), 1);
...
TEST_CASE("apxdby/z/0", "[double][apxdby]") {
    const size_t N = 99;
    ...
    egblas_zapxdby(N, make_cuDoubleComplex(1.0f, 2.0f), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, make_cuDoubleComplex(1.01f, 2.01f), reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);
