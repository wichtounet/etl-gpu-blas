...
TEST_CASE("minus/s/0", "[float][minus]") {
    const size_t N = 137;
...
TEST_CASE("minus/s/1", "[float][minus]") {
    const size_t N = 333;
    ...
    egblas_sminus(N, 0.2f, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2f * -(x_cpu[i])));
    }
...
TEST_CASE("minus/d/0", "[double][minus]") {
    const size_t N = 137;
...
TEST_CASE("minus/d/1", "[double][minus]") {
    const size_t N = 333;
    ...
    egblas_dminus(N, 0.2, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2 * -(x_cpu[i])));
    }
...
TEST_CASE("minus/c/0", "[float][minus]") {
    const size_t N = 137;
    ...
    egblas_cminus(N, make_cuComplex(1.0f, 0.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, reinterpret_cast<cuComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(-(x_cpu[i])));
    }
...
TEST_CASE("minus/c/1", "[float][minus]") {
    const size_t N = 338;
    ...
    egblas_cminus(N, make_cuComplex(1.0f, 1.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, reinterpret_cast<cuComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(std::complex<float>(1.0f, 1.0f) * -(x_cpu[i])));
    }
...
TEST_CASE("minus/z/0", "[double][minus]") {
    const size_t N = 137;
    ...
    egblas_zminus(N, make_cuDoubleComplex(1.0, 0.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(-(x_cpu[i])));
    }
...
TEST_CASE("minus/z/1", "[double][minus]") {
    const size_t N = 338;
    ...
    egblas_zminus(N, make_cuDoubleComplex(0.1, 2.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(std::complex<double>(0.1, 2.0) * -(x_cpu[i])));
    }
