...
TEST_CASE("log2/s/0", "[float][log2]") {
    ...
    egblas_slog2(N, 1.0, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0f * std::log2(1.0f + i)));
    }
...
TEST_CASE("log2/s/1", "[float][log2]") {
    ...
    egblas_slog2(N, 0.2f, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2f * std::log2(1.1f * (i + 1))));
    }
...
TEST_CASE("log2/d/0", "[double][log2]") {
    const size_t N = 137;
    ...
    egblas_dlog2(N, 1.0, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0 * std::log2(1.0 + i)));
    }
...
TEST_CASE("log2/d/1", "[double][log2]") {
    const size_t N = 333;
    ...
    egblas_dlog2(N, 0.2, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2 * std::log2(1.1 * (i + 1))));
    }
...
TEST_CASE("log2/c/0", "[float][log2]") {
    ...
    egblas_clog2(N, make_cuComplex(1.0f, 0.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, reinterpret_cast<cuComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(std::log2(std::complex<float>(1.0f + i / 1000.0f, 1.0f * i))));
    }
...
TEST_CASE("log2/c/1", "[float][log2]") {
    ...
    egblas_clog2(N, make_cuComplex(1.0f, 1.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, reinterpret_cast<cuComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(std::complex<float>(1.0f, 1.0f) * std::log2(std::complex<float>(1.0f + i / 998.0f, -2.0f * i))));
    }
...
TEST_CASE("log2/z/0", "[double][log2]") {
    const size_t N = 137;
    ...
    egblas_zlog2(N, make_cuDoubleComplex(1.0, 0.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(std::log2(std::complex<double>(1.0 + i / 999.0, -1.1 * i))));
    }
...
TEST_CASE("log2/z/1", "[double][log2]") {
    const size_t N = 338;
    ...
    egblas_zlog2(N, make_cuDoubleComplex(0.1, 2.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(std::complex<double>(0.1, 2.0) * std::log2(std::complex<double>(0.1 + i / 996.0, -2.4 * i))));
    }
