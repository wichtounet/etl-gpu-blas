...
template<typename T>
std::complex<T> ceil(std::complex<T> x){
    return {std::ceil(x.real()), std::ceil(x.imag())};
}

} // end of anonymous namespace

TEST_CASE("ceil/s/0", "[float][ceil]") {
    const size_t N = 137;
    ...
    egblas_sceil(N, 1.0, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0f * std::ceil(1.0f + i)));
    }
...
TEST_CASE("ceil/s/1", "[float][ceil]") {
    ...
    egblas_sceil(N, 0.2f, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2f * std::ceil(1.1f * (i + 1))));
    }
...
TEST_CASE("ceil/d/0", "[double][ceil]") {
    const size_t N = 137;
    ...
    egblas_dceil(N, 1.0, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0 * std::ceil(1.0 + i)));
    }
...
TEST_CASE("ceil/d/1", "[double][ceil]") {
    const size_t N = 333;
    ...
    egblas_dceil(N, 0.2, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2 * std::ceil(1.1 * (i + 1))));
    }
...
TEST_CASE("ceil/c/0", "[float][ceil]") {
    ...
    egblas_cceil(N, make_cuComplex(1.0f, 0.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, reinterpret_cast<cuComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(ceil(std::complex<float>(1.0f + i / 1000.0f, 1.0f * i))));
    }
...
TEST_CASE("ceil/c/1", "[float][ceil]") {
    ...
    egblas_cceil(N, make_cuComplex(1.0f, 1.0f), reinterpret_cast<cuComplex*>(x_gpu), 1, reinterpret_cast<cuComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<float>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<float>(std::complex<float>(1.0f, 1.0f) * ceil(std::complex<float>(i / 998.0f, -2.0f * i))));
    }
...
TEST_CASE("ceil/z/0", "[double][ceil]") {
    const size_t N = 137;
    ...
    egblas_zceil(N, make_cuDoubleComplex(1.0, 0.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(ceil(std::complex<double>(i / 999.0, -1.1 * i))));
    }
...
TEST_CASE("ceil/z/1", "[double][ceil]") {
    const size_t N = 338;
    ...
    egblas_zceil(N, make_cuDoubleComplex(0.1, 2.0), reinterpret_cast<cuDoubleComplex*>(x_gpu), 1, reinterpret_cast<cuDoubleComplex*>(y_gpu), 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(std::complex<double>), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == TestComplex<double>(std::complex<double>(0.1, 2.0) * ceil(std::complex<double>(i / 996.0, -2.4 * i))));
    }
