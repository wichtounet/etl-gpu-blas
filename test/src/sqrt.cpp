...
TEST_CASE("sqrt/s/0", "[float][sqrt]") {
    ...
    egblas_ssqrt(N, 1.0, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0f * std::sqrt(1.0f * i)));
    }
...
TEST_CASE("sqrt/s/1", "[float][sqrt]") {
    ...
    egblas_ssqrt(N, 0.2f, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(float), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2f * std::sqrt(1.1f * (i + 1))));
    }
...
TEST_CASE("sqrt/d/0", "[double][sqrt]") {
    const size_t N = 137;
    ...
    egblas_dsqrt(N, 1.0, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(1.0 * std::sqrt(1.0 * i)));
    }
...
TEST_CASE("sqrt/d/1", "[double][sqrt]") {
    const size_t N = 333;
    ...
    egblas_dsqrt(N, 0.2, x_gpu, 1, y_gpu, 1);

    cuda_check(cudaMemcpy(y_cpu, y_gpu, N * sizeof(double), cudaMemcpyDeviceToHost));

    for (size_t i = 0; i < N; ++i) {
        REQUIRE(y_cpu[i] == Approx(0.2 * std::sqrt(1.1 * (i + 1))));
    }
