...
template <typename T>
__global__ void axdy_3_kernel0(size_t n, T* yy, size_t incyy) {
    auto index  = threadIdx.x + blockIdx.x * blockDim.x;
...
template <typename T>
__global__ void axdy_3_kernel0_flat(size_t n, T* yy) {
    auto index  = threadIdx.x + blockIdx.x * blockDim.x;
...
template <typename T>
void axdy_3_kernel0_run(size_t n, T* yy, size_t incyy) {
    static int blockSize   = 0;
    static int minGridSize = 0;

    if (!blockSize) {
        cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, axdy_3_kernel0<T>, 0, 0);
    }

    int gridSize = ((n / incyy) + blockSize - 1) / blockSize;

    if (incyy == 1) {
        axdy_3_kernel0_flat<T><<<gridSize, blockSize>>>(n, yy);
    } else {
