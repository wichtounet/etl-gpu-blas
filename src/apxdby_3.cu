...
#include "egblas/apxdby_3.hpp"

#include "complex.hpp"

template <typename T>
__global__ void apxdby_3_kernel(size_t n, const T alpha, const T* x, size_t incx, T beta, const T* y, size_t incy, T* yy, size_t incyy) {
...
template <typename T>
void apxdby_3_kernel_run(size_t n, T alpha, const T* x, size_t incx, T beta, const T* y, size_t incy, T* yy, size_t incyy) {
    int blockSize;
    int minGridSize;

    cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, apxdby_3_kernel<T>, 0, 0);

    int gridSize = ((n / incy) + blockSize - 1) / blockSize;

    apxdby_3_kernel<T><<<gridSize, blockSize>>>(n, alpha, x, incx, beta, y, incy, yy, incyy);
...
void egblas_sapxdby_3(size_t n, float alpha, const float* x, size_t incx, float beta, const float* y, size_t incy, float* yy, size_t incyy) {
    apxdby_3_kernel_run(n, alpha, x, incx, beta, y, incy, yy, incyy);
}
