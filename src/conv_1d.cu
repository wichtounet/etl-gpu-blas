...
template <typename T>
__global__ void conv1_full_kernel(size_t N, size_t K, T alpha, const T* x, size_t incx, const T * k, size_t inck, T* y, size_t incy) {
    const auto index  = threadIdx.x + blockIdx.x * blockDim.x;

    if (index < N + K - 1) {
        T value(0);

        size_t l_lo = max<int>(0, index - (K - 1));
        size_t l_hi = min<int>(index, N - 1) + 1;

        for (size_t l = l_lo; l < l_hi; ++l) {
            value += x[l * incx] * k[(index - l) * inck];
        }
...
template <typename T>
__global__ void conv1_full_kernel_flat(size_t N, size_t K, T alpha, const T* x, const T * k, T* y) {
    const auto index  = threadIdx.x + blockIdx.x * blockDim.x;
...
template <typename T>
__global__ void conv1_full_kernel1(size_t N, size_t K, const T* x, size_t incx, const T * k, size_t inck, T* y, size_t incy) {
    const auto index  = threadIdx.x + blockIdx.x * blockDim.x;
...
template <typename T>
__global__ void conv1_full_kernel1_flat(size_t N, size_t K, const T* x, const T * k, T* y) {
    const auto index  = threadIdx.x + blockIdx.x * blockDim.x;
...
template <typename T>
void conv1_full_kernel_run(size_t N, size_t K, T alpha, const T* x, size_t incx, const T * k, size_t inck, T* y, size_t incy) {
    int blockSize;
    int minGridSize;

    cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, conv1_full_kernel<T>, 0, 0);

    int gridSize = ((N + K - 1) + blockSize - 1) / blockSize;

    if (alpha == T(1)) {
        if (incx == 1 && inck == 1 && incy == 1) {
            conv1_full_kernel1_flat<T><<<gridSize, blockSize>>>(N, K, x, k, y);
        } else {
            conv1_full_kernel1<T><<<gridSize, blockSize>>>(N, K, x, incx, k, inck, y, incy);
        }
    } else {
        if (incx == 1 && inck == 1 && incy == 1) {
            conv1_full_kernel_flat<T><<<gridSize, blockSize>>>(N, K, alpha, x, k, y);
        } else {
            conv1_full_kernel<T><<<gridSize, blockSize>>>(N, K, alpha, x, incx, k, inck, y, incy);
        }
    }
...
void egblas_sconv1_full(size_t N, size_t K, float alpha, const float* x, size_t incx, const float * k, size_t inck, float* y, size_t incy) {
    conv1_full_kernel_run(N, K, alpha, x, incx, k, inck, y, incy);
}

void egblas_dconv1_full(size_t N, size_t K, double alpha, const double* x, size_t incx, const double * k, size_t inck, double* y, size_t incy) {
    conv1_full_kernel_run(N, K, alpha, x, incx, k, inck, y, incy);
}
