...
template <typename T>
__global__ void batch_k_minus_scale2_kernel(size_t B, size_t K, const T* x, const T* gamma, const T* beta, T* y) {
...
template <typename T>
void egblas_batch_k_minus_scale2_run(size_t b, size_t k, const T* x, const T* gamma, const T* beta, T* y) {
    int blockSize;
    int minGridSize;

    cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, batch_k_minus_scale2_kernel<T>, 0, 0);

    int gridSize = ((b * k) + blockSize - 1) / blockSize;

    batch_k_minus_scale2_kernel<<<gridSize, blockSize>>>(b, k, x, gamma, beta, y);
...
void egblas_sbatch_k_minus_scale2(size_t b, size_t k, const float* x, const float* gamma, const float* beta, float* y) {
    egblas_batch_k_minus_scale2_run(b, k, x, gamma, beta, y);
}
...
template <typename T>
__global__ void batch_k_minus_scale4_kernel(size_t B, size_t K, size_t M, size_t N, const T* x, const T* gamma, const T* beta, T* y) {
...
template <typename T>
void egblas_batch_k_minus_scale4_run(size_t b, size_t k, size_t m, size_t n, const T* x, const T* gamma, const T* beta, T* y) {
    int blockSize;
    int minGridSize;

    cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, batch_k_minus_scale4_kernel<T>, 0, 0);

    int gridSize = ((b * k * m * n) + blockSize - 1) / blockSize;

    batch_k_minus_scale4_kernel<<<gridSize, blockSize>>>(b, k, m, n, x, gamma, beta, y);
...
void egblas_sbatch_k_minus_scale4(size_t b, size_t k, size_t m, size_t n, const float* x, const float* gamma, const float* beta, float* y) {
    egblas_batch_k_minus_scale4_run(b, k, m, n, x, gamma, beta, y);
}
