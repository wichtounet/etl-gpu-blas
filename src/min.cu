...
__device__ cuComplex min(cuComplex x, cuComplex y){
    if (x.x < y.x) {
        return x;
    } else if (y.x < x.x) {
        return y;
...
__device__ cuDoubleComplex min(cuDoubleComplex x, cuDoubleComplex y){
    if (x.x < y.x) {
        return x;
    } else if (y.x < x.x) {
        return y;
...
template <typename T>
__global__ void min3_kernel(size_t n, T alpha, const T* a, size_t inca, const T* b, size_t incb, T* y, size_t incy) {
    auto index  = threadIdx.x + blockIdx.x * blockDim.x;
    auto stride = blockDim.x * gridDim.x;

    for (; index < n; index += stride) {
        y[incy * index] = alpha * min(a[inca * index], b[incb * index]);
    }
}
...
template <typename T>
__global__ void min3_kernel1(size_t n, const T* a, size_t inca, const T* b, size_t incb, T* y, size_t incy) {
    auto index  = threadIdx.x + blockIdx.x * blockDim.x;
    auto stride = blockDim.x * gridDim.x;

    for (; index < n; index += stride) {
        y[incy * index] = min(a[inca * index], b[incb * index]);
    }
}
...
template <typename T>
void min_kernel_run(size_t n, T alpha, const T* a, size_t inca, const T* b, size_t incb, T* y, size_t incy) {
    int blockSize;
    int minGridSize;

    cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, min3_kernel<T>, 0, 0);
...
template <typename T>
void min_kernel1_run(size_t n, const T* a, size_t inca, const T* b, size_t incb, T* y, size_t incy) {
    int blockSize;
    int minGridSize;

    cudaOccupancyMaxPotentialBlockSize(&minGridSize, &blockSize, min3_kernel1<T>, 0, 0);
...
void egblas_smin(size_t n, float alpha, const float* a, size_t inca, const float* b, size_t incb, float* y, size_t incy) {
    if (alpha == 1.0f) {
        min_kernel1_run(n, a, inca, b, incb, y, incy);
    } else if (alpha == 0.0f) {
        min_kernel0_run(n, y, incy);
    } else {
        min_kernel_run(n, alpha, a, inca, b, incb, y, incy);
    }
}
